...
export class DischargeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //     }
    //     this.dischargeSummaryForm.patchValue(data.object);        
    //   } else {
  ...
  addPrescriptionFormBuilder() {
    this.dischargeSummaryForm = this.fb.group({
      ...
      dietPlan: [null, [Validators.required]],
      postSurgery: [null, [Validators.required]],
  ...
  //     this.dischargeSummaryForm.patchValue({ prescriptionId: this.prescriptionId, doctorName: this.doctorDetails, appointment: this.appointmentDetails, patient: this.patientDetails, date: this.date })
  //     if (this.dischargeSummaryForm.valid) {
  //       this.appComponent.startSpinner("Saving data..\xa0\xa0Please wait ...");
  //       this.prescriptionService.updatePrescriptionDetails(this.dischargeSummaryForm.value).subscribe(
  //         (resp: any) => {
  //           if (resp.success) {
  //             alert(resp.message);

...
export class PostsurgeryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //     }
    //     this.addPostSurgeryForm.patchValue(data.object);        
    //   } else {
  ...
  addPrescriptionFormBuilder() {
    this.addPostSurgeryForm = this.fb.group({
      ...
      surgerySummary: [null, [Validators.required]],
  ...
  prescriptionDetails(): boolean {
    ...
    this.addPostSurgeryForm.patchValue({
  ...
  //     this.addPostSurgeryForm.patchValue({ prescriptionId: this.prescriptionId, doctorName: this.doctorDetails, appointment: this.appointmentDetails, patient: this.patientDetails, date: this.date })
  //     if (this.addPostSurgeryForm.valid) {
  //       this.appComponent.startSpinner("Saving data..\xa0\xa0Please wait ...");
  //       this.prescriptionService.updatePrescriptionDetails(this.addPostSurgeryForm.value).subscribe(
  //         (resp: any) => {
  //           if (resp.success) {
  //             alert(resp.message);
